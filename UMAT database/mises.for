...
C
 ...
 1       FORMAT(//,30X,'***ERROR - THIS UMAT MAY ONLY BE USED FOR ',
     1          'ELEMENTS WITH THREE DIRECT STRESS COMPONENTS')
      ENDIF
C
C     ---------- Read material parameters and Calculate elastic material constants ----------
C
      EMOD=PROPS(1)
      ENU=PROPS(2)
...
C
C     ---------- Initialize elastic stiffness matrix DDSDDE ----------
      DO 20 K1=1,NTENS
        DO 10 K2=1,NTENS
           DDSDDE(K2,K1)=0.0
 10     CONTINUE
...
C
 ...
 50   CONTINUE
C
C     ---------- Calculate stress increment from elastic strains ----------
C
      DO 70 K1=1,NTENS
        DO 60 K2=1,NTENS
 ...
 70   CONTINUE
C
C     ---------- Recover elastic and plastic strain components from state variables ----------
C
      DO 80 K1=1,NTENS
...
C
C ---------- Check if plasticity is active  ----------
C
...
C
 ...
 120      CONTINUE
C
C       ---------- Newton-Raphson iteration to solve for plastic multiplier ----------
C
          SYIELD=SYIEL0
          DEQPL=0.0
...
C
C       ---------- Update stress, elastic strain, plastic strain, and equivalent plastic strain ----------
C
          DO 150 K1=1,NDI
...
C
C       ---------- Compute consistent tangent stiffness (Jacobian) ----------
C
 ...
 250      CONTINUE
      ...
      ENDIF
C
C     ---------- Save updated elastic and plastic strains to state variable array ----------
C
      DO 310 K1=1,NTENS
        STATEV(K1)=EELAS(K1)
...
C
 ...
 1              FORMAT(//,30X,'***ERROR - PLASTIC STRAIN MUST BE ',
     1                 'ENTERED IN ASCENDING ORDER')
              ...
              ENDIF
C
C           ---------- Interpolate current yield stress and hardening modulus ----------
C
            DEQPL=EQPL1-EQPL0
            SYIEL0=TABLE(1,K1)
            SYIEL1=TABLE(1,K1+1)
